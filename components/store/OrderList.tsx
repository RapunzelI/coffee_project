...
import { Card, List, Tag, Button, Divider } from 'antd';
    ...
    <div className='mb-5'>
      ...
      {confirmedOrders.length > 0 && (
        <>
          ...
          <h5 className="text-lg font-semibold mb-3">ออเดอร์ยืนยันแล้ว</h5>
          <List
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex gap-2 items-center">
                      ...
                      <Tag color="blue">ยืนยันแล้ว</Tag>
                    </div>
                    <p className="mt-2">{order.customerText}</p>
                    <p className="font-bold text-[#C67C4E]">{order.price} ฿</p>
                  </div>
                  <Button type="primary" onClick={() => onStartPreparing(order)}>
                    เริ่มทำ
                  </Button>
                </div>
              </Card>
      ...
      {preparingOrders.length > 0 && (
        <>
          {(pendingOrders.length > 0 || confirmedOrders.length > 0) && <Divider />}
